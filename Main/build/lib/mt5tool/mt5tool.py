...
def auto_sl(sym,  type, percent_equity = 1):
...
def auto_tp(sym, type, percent_equity = 1):
    pe = percent_equity / 100
    sym = sym.upper()
    type = type.lower()
    Buyyers : float = mt5.symbol_info_tick(sym)._asdict()['bid'] # Buyyers
...
def update_sl(symbol, ):
    ...
    if mt5.positions_get(symbol=symbol):
        try:
            ...
            diff = abs(sl_now - price_now)
            print(diff)
            sl_new = sl_now
            if pos_type == 1:  # SELL
                sl_new = sl_now - ((diff / 2) + spread)
...
def update_tp(symbol):
    ...
    if mt5.positions_get(symbol=symbol):
        try:
            ...
            spread, pip = Symbol_info(symbol)
            diff = abs(price_now - open_price)

            tp_new = tp_now
            if pos_type == 1:  # SELL
                tp_new = tp_now - (diff / 2)
...
def market_order(*, symbol: str, time_frame: str,
    ...
    if order_type == 'buy':
        if sl == "amount":
            sl = auto_sl(symbol, order_type, sl_rate)
        if sl == "step":
            sl = price_dict[order_type]  - (POINT* sl_rate)  - SPREAD

        if tp == "amount":
            tp = auto_tp(symbol,  order_type, tp_rate)
        if tp == "step":
            tp = price_dict[order_type] + (tp_rate * POINT) + SPREAD


    # SELL
    elif order_type == 'sell':
        if sl == "amount":
            sl = auto_sl(symbol, time_frame, order_type)
        if sl == "step":
            sl = price_dict[order_type] + SPREAD + (POINT * sl_rate)
